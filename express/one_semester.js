...
module.exports = {
    get_one_semester: function get_one_semester(prev_semester_nodes_list, field) {
        ...
        for (var i = 0; i < prev_semester_nodes_list.length; i++) {
            ...
            for (var j = 0; j < child_nodes.length; j++) {
                ...
                let child_node_idx = curr_semester_nodes_list.findIndex(element => element == child_nodes[j])
    ...
    get_child_nodes: function get_child_nodes(prev_course_node, field) {
    ...
    select_from_eligible: function select_from_eligible(prev_course_status, eligible_courses) {
        ...
        for (var i = 0; i < eligible_ids.length; i++) {
            ...
            for (var j = i+1; i < eligible_ids.length; j++) {
                ...
                if (conflict_ids[i].includes(course2_id)) { continue; }
                
                for (var k = j+1; k < eligible_ids.length; k++) {
                    let course3_id = eligible_ids[k];
                    if (conflict_ids[i].includes(course3_id)) {continue; }
                    if (conflict_ids[j].includes(course3_id)) {continue; }
