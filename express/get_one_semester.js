...
function get_child_nodes(prev_course_node, field) {
    ...
    return filter.filterByPre(prev_course_status, field, get_no_conflict_courses_from_eligible) // for function callback
}

function get_no_conflict_courses_from_eligible(prev_course_node, eligible_courses) {
        ...
        for (var j = i+1; i < eligible_ids.length; j++) {
            ...
            for (var k = j+1; k < eligible_ids.length; k++) {
                ...
                if (conflict_ids[j].includes(course3_id)) {
                    ...
                    let child_course_status = prev_course_node.get_status();
